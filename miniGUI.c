...
void EPD_DrawRGBPixel(uint16_t x0, uint16_t y0, uint8_t color)
{
	uint32_t x = x0, y = y0;

	x = Sys_info.uiWidth - y0;
	y = x0;
	if(x < 0 || x >= Sys_info.uiWidth || y < 0 || y >= Sys_info.uiHeight)
		return ;	
	/*
	显存处理
	*/
...
void EPD_DrawPixel(uint16_t x0, uint16_t y0, uint8_t color)
{
	...
	显存处理
	*/
	gpFrameBuf[y0*Sys_info.uiWidth + x0] = color;
}
...
void EPD_Text(uint16_t Xpos,uint16_t Ypos,uint8_t *str,uint8_t Color,uint8_t bkColor)
{
	...
	do
	{
		...
		if(Xpos < Sys_info.uiWidth - 8)
		{
			Xpos += 8;
		} 
		else if (Ypos < Sys_info.uiHeight - 16)
		{
			Xpos = 0;
			Ypos += 16;
		}   
...
static int ScreenTransition(uint8_t *dst, uint8_t *src, uint32_t w, uint32_t h) {
	uint32_t i,j,k;
    uint8_t *psrc = src;
    uint8_t *pdst = dst;
    uint8_t *p = psrc;
